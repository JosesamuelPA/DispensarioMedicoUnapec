...
El sistema cuenta con un módulo de seguridad. Para entrar por primera vez:
1.  Ve a la pantalla de **Registro** (`/Auth/Register`).
2.  Crea tu usuario y contraseña.
3.  Inicia sesión en la pantalla de **Login** (`/Auth/Login`).

---

## 💡 Datos de Prueba
El sistema incluye un inicializador automático (`DbInitializer.cs`). Al ejecutar la aplicación por primera vez (y tras haber aplicado las migraciones):
- Se crearán **32 pacientes** automáticamente.
- Se cargarán **Marcas**, **Estantes** y **Medicamentos**.
- Se generarán cientos de **Visitas médicas** de prueba para los reportes.
...
- **Frontend**: El proyecto utiliza **Bootstrap 5** y **Select2** a través de CDNs integrados en el Layout. **No es necesario realizar `npm install`**.
- **Iconos**: Bootstrap Icons & Material Symbols
...
Este error ocurre en Windows si intentas compilar mientras la aplicación sigue corriendo en segundo plano o el proceso quedó bloqueado.
- **Solución**: Abre una terminal (PowerShell) y ejecuta:
  ```powershell
  Stop-Process -Name DispensarioMedicoUnapec -Force
  ```
  Luego intenta ejecutar `dotnet run` nuevamente.

---

## 👥 Colaboración
Si vas a realizar cambios:
1.  Crea una rama nueva: `git checkout -b feature/nombre-mejora`
2.  Haz tus cambios y commits descriptivos.
3.  Asegúrate de ejecutar `dotnet build` antes de subir tus cambios.